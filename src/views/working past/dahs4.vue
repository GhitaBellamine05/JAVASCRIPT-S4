<template>
  ...
  import { 
    ...
    getDownloadURL, uploadBytes, Timestamp 
  ...
  export default {
    ...
    setup() {
      ...
      const projectStats = ref({ active: 0, completed: 0, completionRate: 0 });
      const skillStats = ref({ mastered: 0, learning: 0, masteryRate: 0 });
      const objectiveStats = ref({ completed: 0, inProgress: 0, completionRate: 0 });
      const communityStats = ref({ posts: 0, followers: 0, engagementRate: 0 });
      const activityStats = ref({ weekly: 0, monthly: 0, total: 0 });
      const notifications = ref({ total: 0, messages: 0, community: 0 });
      ...
      const initFirebaseListeners = () => {
        ...
        onSnapshot(userRef, (doc) => {
          if (doc.exists()) {
            user.value = { id: userId, ...doc.data() };
          }
        });
        
        // User stats
        const statsRef = doc(db, 'users', userId, 'private', 'stats');
        onSnapshot(statsRef, (doc) => {
          if (doc.exists()) {
            ...
            objectiveStats.value = data.objectiveStats || {};
            communityStats.value = data.communityStats || {};
        ...
        const projectsQuery = query(
          ...
          where('status', 'in', ['in-progress', 'almost-done']),
          orderBy('deadline', 'asc'),
          limit(3)
        );
        onSnapshot(projectsQuery, (snapshot) => {
          activeProjects.value = snapshot.docs.map(doc => ({
            id: doc.id,
            ...doc.data(),
        ...
        onSnapshot(skillsQuery, (snapshot) => {
          topSkills.value = snapshot.docs.map(doc => ({
            id: doc.id,
            ...doc.data()
          }));
        });
        ...
        onSnapshot(objectivesQuery, (snapshot) => {
          recentObjectives.value = snapshot.docs.map(doc => ({
            id: doc.id,
            ...doc.data(),
            deadline: doc.data().deadline?.toDate()
          }));
        ...
        onSnapshot(postsQuery, (snapshot) => {
          communityPosts.value = snapshot.docs.map(doc => ({
            ...
            isLiked: doc.data().likes?.includes(userId) || false,
            showComments: false,
            newComment: ''
          }));
        });
        ...
        onSnapshot(followersQuery, async (snapshot) => {
          const followers = [];
          for (const doc of snapshot.docs) {
            ...
            if (followerDoc.exists()) {
              followers.push({
                id: followerId,
                ...followerDoc.data(),
                isFollowing: doc.data().isFollowing || false
              });
            }
          }
          recentFollowers.value = followers;
        });
        ...
        onSnapshot(notificationsQuery, (snapshot) => {
          notificationList.value = snapshot.docs.map(doc => ({
            id: doc.id,
            ...doc.data(),
            time: doc.data().time?.toDate()
          }));
          notifications.value.total = notificationList.value.length;
        });
      };
      ...
      const dismissNotification = async (id) => {
        try {
          await updateDoc(doc(db, 'users', user.value.id, 'notifications', id), {
            read: true
          });
        } catch (error) {
      ...
      const markAllAsRead = async () => {
        try {
          ...
          notificationList.value.forEach(notification => {
            const notificationRef = doc(db, 'users', user.value.id, 'notifications', notification.id);
        ...
        try {
          ...
          if (post.isLiked) {
            ...
            await updateDoc(postRef, {
              likes: arrayRemove(user.value.id)
            });
          } else {
            // Add like
            await updateDoc(postRef, {
              likes: arrayUnion(user.value.id)
            });
          }
        } catch (error) {
      ...
      const toggleFollow = async (followerId) => {
        try {
          const followerRef = doc(db, 'users', user.value.id, 'followers', followerId);
          ...
          if (follower.isFollowing) {
            ...
            await updateDoc(doc(db, 'users', user.value.id, 'private', 'stats'), {
              'userStats.following': increment(-1)
            });
          } else {
            // Follow
            await updateDoc(followerRef, {
      ...
      const handleFileUpload = async (e) => {
        ...
        try {
          ...
          const storageRef = ref(storage, `posts/${user.value.id}/${Date.now()}_${file.name}`);
          
          // Upload the file
          uploadTask.value = uploadBytes(storageRef, file);
          const snapshot = await uploadTask.value;
      ...
      const submitPost = async () => {
        ...
        try {
          ...
          if (postImage.value) {
            postData.image = postImage.value;
          }
          ...
          await updateDoc(doc(db, 'users', user.value.id, 'private', 'stats'), {
            'communityStats.posts': increment(1)
          });
      ...
      onMounted(() => {
        ...
        // Watch for changes in data that affects charts
        watch([topSkills, activityStats], () => {
          initCharts();
        }, { immediate: true });
      });
