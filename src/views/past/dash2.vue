  ...
  methods: {
    setupAuthListener() {
      onAuthStateChanged(auth, async (firebaseUser) => {
        if (firebaseUser) {
          ...
          this.user.photoURL = firebaseUser.photoURL || '/default-avatar.png'
          ...
          if (userSnap.exists()) {
            ...
            this.user.name = userData.name || firebaseUser.displayName || 'New User'
            this.userStats = userData.stats || this.userStats
            this.user.isNewUser = false
          } else {
            // New user - initialize their data
            this.user.name = firebaseUser.displayName || 'New User'
    ...
    async likePost(postId) {
      ...
      await updateDoc(postRef, {
        likes: this.communityPosts.find(p => p.id === postId).likes + 1
      })
    },
    ...
    async createNewPost() {
      try {
        ...
        } else if (this.newPostType === 'activity') {
          newPost.activityType = this.newActivity.type
        }
    ...
    startEditProfile() {
      this.tempProfileData = {
        name: this.user.name,
        title: 'Software Developer', // You might want to store this in user data
        photoURL: this.user.photoURL
      }
    ...
    async saveProfile() {
      try {
        ...
        await updateDoc(userRef, {
          ...
          title: this.tempProfileData.title,
          photoURL: this.tempProfileData.photoURL,
          updatedAt: serverTimestamp()
        })
        
        // Update local user data
        this.user.name = this.tempProfileData.name
        this.user.photoURL = this.tempProfileData.photoURL
