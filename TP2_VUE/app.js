    ...
    computed: {
        personnesFiltres() {
            return this.personnes.filter(p => {
                const genreValide = (this.homme && p.gender === 'male') || (this.femme && p.gender === 'female');
                const nomValide = p.name.last.toLowerCase().includes(this.filtreNom.toLowerCase());
                return genreValide && nomValide;
            });
        }
    },
